...
class Search extends Component {
  ...
  async componentDidMount() {
    ...
    const search = await postService(data, "search_tourist")
    if(search.meta.code==200){
      this.setState({
        searchAll:search.data
      })
    }else{
      this.setState({
        searchAll:[]
  ...
  search = () => {
    return this.state.searchAll.map((value, index) => {
      return (
        ...
        <View style={{ height: 100, width: "100%", flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
          <View style={{ width: "80%" }}>
            <Text style={{
              ...
              elevation: 5,
            }}> Hasil Search Key "{this.props.route.params.name}" </Text>
          </View>
          <TouchableOpacity onPress={() => this.props.navigation.navigate("AddItinerary",{category_id:1})}>
            <Image
